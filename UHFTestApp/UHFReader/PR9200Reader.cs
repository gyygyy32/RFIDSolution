...
namespace UHFReader
{
    ...
    public delegate void ReadTIDBankWhenWriteFailDelegate(ActionResault resaultType);
    ...
    public class PR9200Reader 
    {
        ...
        public GetFrequencyRegionDelegate getFrequencyRegionCallback;
        public ReadTIDBankWhenWriteFailDelegate readTIDBankWhenWriteFailDelegate;
        ...
        public void WriteTag(byte[] tbWriteData)
        {
            ...
            actionType = ActionType.ReadTIDBankWhenWrite;
        ...
        private void ReceivedComData(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            try
            {
                ...
                ParseResponsePacket(readBuffer);
            }
            catch 
            {
        ...
        private void ParseResponsePacket(byte[] btAryReceiveData)
        {
            ...
            switch (actionType)
            {
                ...
                case ActionType.GetFrequencyRegion:
                    ...
                    {
                        ProcessGetFrequencyRegion(lstResponsePacket[0]);
                    }
                    else
                    {
                        SetErrorCode("返回数据包格式不正确，请重试！");
                        getFrequencyRegionCallback(ActionResault.GetFrequencyRegionFail);
                    }
                    break;
            ...
            if (lstResponsePacket.Count==0)
            {
                
            }
            else if (lstResponsePacket.Count > 1)
            {

            }
            else {
                /*
                 * just make sure only one tag been writen
                 */
                PR9200Msg msgTran = lstResponsePacket[0];
        ...
        #region reader response packet process functions
        ...
        private void ReadTagBeforeWriteCallBack(PR9200Msg msgTran)
        {
            string strErrorCode = string.Empty;
        ...
        private void WriteTagCallBack(PR9200Msg msgTran)
        {
            string strErrorCode = string.Empty;
        ...
        private void ProcessGetFrequencyRegion(PR9200Msg msgTran)
        {
            ...
            {//system frequencies used
                ...
                getFrequencyRegionCallback(ActionResault.GetFrequencyRegionSuccess);
                //WriteLog(lrtxtLog, strCmd, 0);
                return;
            }
        ...
        private void WriteData2Tag(string data)
        {
            try
            {
                byte btMemBank = 0x00;
                ...
                if (m_targetMemoryBank == 1)
                {
                    btMemBank = MemoryBank.EPCBank;
                }
                else if (m_targetMemoryBank == 2)
                {
                    btMemBank = MemoryBank.USERBank;
                }
                else
                {
                    //WriteLog(lrtxtLog, "target memry bank not correct!", 1);
                    return;
                }
        ...
        private void GetTIDWhenRead()
        {
            try
            {
                //action_Queue.Enqueue(ActionType.ReadTIDBankWhenRead);

                byte wordoffset = 0x00;
                byte wordCnt = 0x02;
                TagManger.reader.ReadTag(PR9200Setting.btReadId, MemoryBank.TIDBank, wordoffset, wordCnt);
            }
            catch (System.Exception ex)
            {
                //MessageBox.Show(ex.Message);
